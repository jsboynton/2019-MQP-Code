...
void setup() { // setup code here, to run once: 
  ...
  unsigned int   configWord; // setting up definitions for power and direction   
  ...
  // set to max current limit and disable ISR slew limiting configWord = 0b0000010000001100; 
...
// IMPORTANT: this is the code that automatically places the motor in the home position when powered on/booting up the GUI   
  dir1=0;   
  pwm1=255;   
  ...
  delay(5000); } // End setup void loop() 
  {  
    if( Serial.available()>0) {    // following code is for reading what information it was sent from GUI to arduino 
  ...
  int selectorvalue = (int)values[3]; //variable that will be used to select case to perform a specific action, all these value were taken from GUI 
  ...
  fieldIndex = 0;  // Reset the field index - ready to start over switch (selectorvalue){ //selects which case to run based on selector value case 1:   //IMPORTANT: code for running strain and stop actions (stop program has power input values for x and y strain as 0, so actuators don't move during either section of this code) while(Serial.available() ==0){   while(i==1){ 
//all actuators extend in this step 
  ...
  digitalWrite(DIR_M1, dir1); 
  analogWrite(PWM_M1, pwm1); // write to pins, tells motors to run at the set speed and direction 
  dir2 = 1; 
  pwm2 = yStrainValue; 
  digitalWrite(DIR_M2, dir2); 
  analogWrite(PWM_M2, pwm2); 
  ...
  Serial.println(cycleCount); //send number of cycles to GUI to be displayed 
   ...
   break; case 2:  //IMPORTANT: code for running home and stop action 
  ...
  digitalWrite(DIR_M4, dir4); 
  analogWrite(PWM_M4, pwm4); 
   delay(timePerCycle*500);  // length of time motors run this step for break; 
  ...
  digitalWrite(DIR_M3, dir3); 
  analogWrite(PWM_M3, pwm3); 
  delay(timePerCycle*500);  // length of time until motors run this step for i=i+1; 
} while (i==2){ 
